\documentclass[type=bachelor, fontset=fandol, seriftoc, printmode=twoside]{cquthesis}

% 可用选项：
% type=[bachelor|master|doctor],      % 必选，毕业论文类型，以下项目不填时为默认
% liberalformat,                      % 可选，仅适用本科生，使用文学类论文标题格式，默认未打开
% proffesionalmaster=[true|false],    % 可选，仅适用研究生，是(true)否(false)专业硕士，默认为否
% printmode=[oneside|twoside|auto],	  % 可选，论文打印方式，默认采用auto按页数要求自动判定
% openany,|openright,                 % 可选，双面打印时每章的第一页仅右页开启，默认右页开启(openright)
% bilinguallist=[off|combined|apart], % 可选，图录表录等分别按双语题注混编(combined)，分开编录(apart)，默认关(off)
% blindtrail,                         % 可选，盲审模式，开启后封面姓名和致谢部分会隐藏，详情请参阅用户文档，默认关
% draft,                              % 写作期间可选，不渲染图片，关闭外围功能，加快预览速度，默认未开启

% 请在cquthesis.sty文件中定义其他会用到的宏包和自己的变量
% 这样可以防止main.tex太过臃肿。
\usepackage{cquthesis}
% 在 oneside 下强制 \cleardoublepage 补到奇数页（须用 \renewcommand，不能用 \newcommand）
\makeatletter
\renewcommand{\cleardoublepage}{%
  \clearpage
  \ifodd\c@page
  \else
    \null
    \ifcqu@blankleft\thispagestyle{style@empty}\fi
    \newpage
    \if@twocolumn
      \null\newpage
    \fi
  \fi
}
\makeatother
%%%
\usepackage{ctex}        % 中文支持

\ctexset{
  section = { beforeskip = 24bp, afterskip = 24bp },
  subsection = { beforeskip = 20bp, afterskip = 20bp },
}

\usepackage{graphicx}    % 图片插入
\usepackage{float}       % 图表位置控制[htbp]
\usepackage{booktabs}   % 三线表
\usepackage{array}      % p列支持
\usepackage{makecell}   % 可选，支持单元格换行
\usepackage[section]{placeins}
\usepackage{longtable}
\usepackage{multirow}

\usepackage{caption}
\captionsetup{
    font={zihao{5}},         % 使用small字体（对应五号）
    labelsep=quad,        % 编号和标题之间空一个汉字符
    justification=centering % 标题居中
}
\captionsetup[table]{position=top,belowskip=6bp,aboveskip=0bp}
\captionsetup[figure]{position=bottom,belowskip=0bp,aboveskip=6bp}


\makeatletter
\def\cqu@tabular{\zihao{5}\@tabular}
\makeatother

% 3. 调整表格行高（如果觉得当前行高过大，可以适当调小）
\renewcommand{\arraystretch}{1.2} % 1.2到1.5之间视情况调整

% 定义所有的图片文件在 figures 子目录下
\graphicspath{{figures/}}


% ===== 目录页按 Word 模版 =====
% 1) 「目」与「录」之间空两字符
\renewcommand{\contentsname}{目\hskip2\ccwd 录}

% 2) 标题上下各「空一行」：勿用 1\baselineskip（会被 sty 里 1.667 行距放大，且不等于 Word）
%    Word 空一行 ≈ 一个五号字高的空白，用固定 bp/ccwd 更稳
\setlength{\cftbeforetoctitleskip}{0pt}   % 页眉线到「目录」，仍偏大可试 -12pt
\setlength{\cftaftertoctitleskip}{2\ccwd} % 「目录」到第一条

% 3) 目录条目：黑体五号（章/节/小节统一）
\renewcommand{\cftchapfont}{\zihao{-5}\heiti}
\renewcommand{\cftsecfont}{\zihao{-5}\heiti}
\renewcommand{\cftsubsecfont}{\zihao{-5}\heiti}
\renewcommand{\cftchapleader}{\cftdotfill{\cftchapdotsep}}  % 保留引导点
% over-20260521

\usepackage{setspace}
\setstretch{1.667}

%*** 写作时，使用这个命令只渲染你想查看的部分，提升工作效率，定稿时注释掉整行
% \includeonly{contents/ch3_inflatable}
\begin{document}

\input{contents/cover}
\makecover %%% 封面部分


\frontmatter %%%前置部分(封面后绪论前)
%% 摘要
\makeabstract
%% 目录，注意需要多次编译才能更新
\tableofcontents
% %% 插图索引，可选，如不用可注释掉
% \listoffigures
% \listoffiguresEN
% %% 表格索引，可选
% \listoftables
% \listoftablesEN
% %% 公式索引，可选
% \listofequations
% \listofequationsEN
% %% 符号对照表，可选
% \input{contents/denotation}


\mainmatter %%% 主体部分(绪论开始，结论为止)

\include{contents/ch1_introduction}    % 第一章 绪论
\include{contents/ch2_requirements}    % 第二章 系统需求分析与总体方案
\include{contents/ch3_inflatable}      % 第三章 充气伞形结构设计
\include{contents/ch4_power}           % 第四章 动力系统理论计算与选型
\include{contents/ch5_control}         % 第五章 嵌入式飞行控制系统
\include{contents/ch6_integration}     % 第六章 系统集成与车载平台实现
\include{contents/ch7_experiment}      % 第七章 实验验证与分析
\include{contents/ch8_conclusion}      % 第八章 总结与展望

% \include{contents/yourFreeChoise}


\backmatter %%% 后置部分(致谢、参考文献、附录等)



%% 参考文献
% 顺序编码制：cqunumerical		
% 注意：至少需要引用一篇参考文献，否则下面两行会引起编译错误。
\bibliographystyle{cqunumerical}
\bibliography{ref/refs}

%% 致谢
\include{contents/ack}

%% 附录(按ABC...分节，证明、推导、程序、个人简历等)
\appendix

% 个人简历
% \include{contents/appendix}
%% 原创声明和授权说明书，可选：用扫描页替换
\cquauthpage[contents/authscan.pdf]
% \cquauthpage
\end{document}